...
- Remaining live Supabase blockers: owner reset email is temporarily rate-limited by Supabase email delivery; Google SSO remains intentionally hidden and unconfigured because Google OAuth credentials are absent; `https://praxisgrid.vercel.app` is not assigned; database password rotation is required because a Supabase CLI `db dump --dry-run` printed a database password in local tool output.
