...
- Twenty-first hardening slice advances canonical production identity: the connected Vercel project was renamed from `azure-quest-pwa` to `praxisgrid`, and remote project settings now match Vite, `npm run build`, `dist`, and the repo install command. The old production alias still serves the latest deployment; `praxisgrid.vercel.app` is pending a fresh production deploy/alias.
...
- The Vercel project is now renamed to `praxisgrid`; the historical production alias currently remains `https://azure-quest-pwa.vercel.app` until a fresh production deployment or alias makes `https://praxisgrid.vercel.app` live.
