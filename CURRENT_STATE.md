...
- Senior Product Designer and Principal Engineer reviews on 2026-07-28 keep production sign-off at FAIL. Several original blockers are now fixed or live-verified; remaining open blockers include source-grounded production content, full protected Admin mutations, broader signed-in browser/product smoke, Project Intelligence visual refinement, GitHub import quota live denial tests, production canonical domain assignment, and external auth constraints.
- M5.0 authorization hardening migration `0006_m5_authorization_hardening.sql` was added to separate review/publish authority, block reviewer publication, harden audit actors, and fix imported-project cloud row collisions. Focused live Supabase role/RLS verification now passes for the covered paths.
...
- Repository migrations `0001` through `0025` were dry-run and applied successfully to an empty hosted staging fallback, then dry-run and applied successfully to `praxisgrid-production`; migration `0026_project_intelligence_owner_fk.sql` was dry-run and applied successfully after PR #5; migration `0027_fix_github_import_quota_output_ambiguity.sql` fixed the live GitHub import quota RPC; production local/remote migration history now matches through `0027`.
...
- Live owner bootstrap, focused two-user RLS isolation, role-boundary checks, and focused production authenticated browser verification now pass for the covered M5 surfaces. Google SSO remains intentionally hidden and externally blocked until Google OAuth credentials are supplied.
...
- PR #6 merged local user-isolation hardening into `main`: localForage learner data is partitioned by authenticated Supabase user ID, hydration reruns when the auth user changes, signed-in users cannot fall back to anonymous legacy data, reset is scoped to the active local partition, and `validate:local-user-isolation` is wired into CI. PR CI and merged `main` CI run `30766968811` passed. Production deployment `dpl_3FK5MjhS14Kbo5VdqA8HsTPE3nZC` is READY at `https://praxisgrid-1rk29n3if-tonybabalola-1114s-projects.vercel.app` and aliased to `https://azure-quest-pwa.vercel.app`; public production smoke passed 20/20 with 10 expected signed-in skips.
- PR #7 fixed the live recovery password-update form and merged through green CI. PR #8 added exact Supabase `/auth?mode=update-password` redirect allow-list entries, added immediate recovery-form feedback, pushed the live Supabase auth config, merged through green CI, and redeployed production. Current production deployment `dpl_2yfxrw9Ji7kjvfiahoCntv1sNaRu` is READY at `https://praxisgrid-rg9891u72-tonybabalola-1114s-projects.vercel.app` and aliased to `https://azure-quest-pwa.vercel.app`; merged `main` CI run `30771557919` passed.
- Live disposable-account verification passed: signup confirmation, default `USER` role, sign-in, onboarding persistence, admin denial, logout, re-sign-in, recovery redirect, password update, and rotated-password sign-in.
- Live owner bootstrap passed: `tobibabalola21@gmail.com` was created and verified through a fresh session as `MAIN_ADMIN`. Live personal study account `tonybabalola@gmail.com` was created and verified through a fresh session as `USER`.
- Live role/RLS verification passed for the covered M5 surfaces: all returned public tables have RLS enabled; temporary User A/User B isolation passed 27 read/update/delete checks in each direction across profiles, assessment sessions, quiz attempts, question flags, imported projects, Project Intelligence analyses, interview sessions, active interview sessions, and content quality reports; USER cannot escalate roles or read role audit; CONTENT_REVIEWER and SUPPORT_ADMIN cannot publish approved questions; SUPPORT_ADMIN can read support reports; MAIN_ADMIN can read role audit; `/admin` browser access allows the three admin roles and denies the study USER.
- Temporary live role/RLS QA users were deleted after verification. Temporary content-quality report rows created by the live QA run were explicitly removed because that table uses `on delete set null`.
- Branch `codex/m5-live-supabase-qa-scripts` adds opt-in live production scripts: `validate:live-supabase`, `test:auth-production`, `test:live-rls`, `test:roles-production`, and `test:repository-isolation-production`. The full `validate:live-supabase` script passed against `praxisgrid-production` with explicit production opt-in and exact project guards, then deleted all temporary users.
- Branch `codex/m5-live-github-import-quota` extends the live harness with `test:github-import-production`, found and fixed a live ambiguous-column defect in `claim_github_import_quota`, applied migration `0027` to production, verified local/remote migration history through `0027`, and passed the full opt-in `validate:live-supabase` run including GitHub import quota checks.
- PR #12 merged the live GitHub import quota RPC fix through green CI. Merged `main` CI run `30774959344` passed for commit `17bd7dad`, and production deployment `dpl_6N7bi3rzhVzmWLoxRmwFDzearukZ` is READY at `https://praxisgrid-jd3v3rgow-tonybabalola-1114s-projects.vercel.app`, aliased to `https://azure-quest-pwa.vercel.app`; public production smoke passed 20/20 with 10 expected signed-in production skips.
- Branch `codex/m5-production-browser-smoke` adds `test:browser-production-auth` for opt-in live production browser QA. The live run passed against `https://azure-quest-pwa.vercel.app` with temporary users: learner onboarding/admin denial/trust-copy checks passed on 1280x900 and 390x844, SUPPORT_ADMIN could inspect reports without approve actions, and all temporary users were deleted.
- Remaining live Supabase blockers: owner reset email has been sent and owner completion of the reset link remains pending; Google SSO remains intentionally hidden and unconfigured because Google OAuth credentials are absent; `https://praxisgrid.vercel.app` is not assigned; database password rotation is required because a Supabase CLI `db dump --dry-run` printed a database password in local tool output.
...
- Authorization/RLS foundation defects for reviewer publication, approved-serving integrity, caller-shaped audit fields, and imported-project ID collisions are fixed in-repo and statically retested. Focused live Supabase RLS verification now passes for role boundaries, user-owned tables, imported projects, Project Intelligence analyses, and role-change audit forgery protection; broader privileged mutation audit coverage remains pending.
...
- `/admin` review studio exists and is now route-protected in repo. Focused live role-specific browser verification passed for USER denial and MAIN_ADMIN, CONTENT_REVIEWER, and SUPPORT_ADMIN access; full protected Admin queue/mutation integration remains pending.
...
- GitHub import quota RPC is atomic, service-role-scoped, and live-verified: service-role claims work, daily-limit denial works, and anon/authenticated RPC execution is denied.
- Project Intelligence analysis rows are user-scoped in repo; focused live two-user repository-isolation verification passed.
...
- Latest design/security reviews require remaining source-grounded production content, broader browser verification, Project Intelligence visual refinement, live Admin operations, GitHub import quota live denial tests, and production canonical deployment before sign-off.
- Public production smoke passed 20 public checks against `https://azure-quest-pwa.vercel.app` after the Vercel project rename; 6 dev-only signed-in harness checks skipped as intended outside localhost. Focused live Supabase/RLS, role-specific Admin, onboarding, recovery verification, and live production browser auth smoke now pass; broader cross-device sync and source-grounded content verification remain pending.
