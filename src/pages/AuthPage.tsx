...
export function AuthPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-[var(--aq-ink)] dark:bg-[#061227]">
      <section className="mx-auto grid min-h-screen w-full max-w-6xl items-center gap-8 px-4 py-10 md:grid-cols-[0.9fr_1.1fr]">
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          <Badge className="border-[var(--aq-blue-600)] bg-[var(--aq-blue-700)] text-white">{PRODUCT_NAME}</Badge>
          <div>
            <h1 className="max-w-xl text-4xl font-bold leading-tight text-slate-950 dark:text-white sm:text-5xl">{PRODUCT_TAGLINE}</h1>
            <p className="mt-4 max-w-lg text-base font-medium leading-7 text-slate-600 dark:text-slate-300">
        ...
        <Card className="border-[var(--aq-border)] bg-white shadow-[var(--aq-shadow)] dark:bg-[#0b1b33]">
          <CardHeader>
            <div>
              <Badge className="mb-2">{mode === "signup" ? "Signup" : mode === "reset" ? "Recovery" : "Signin"}</Badge>
              <CardTitle>{title}</CardTitle>
              <p className="mt-2 text-sm font-semibold text-[var(--aq-muted)]">Google and email sign-in use Supabase. Your account controls access to Learn, Practise, and Prove.</p>
            </div>
          ...
          <CardContent>
            ...
            <div className="mb-4 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--aq-muted)]">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm font-semibold">
              {mode !== "signin" ? <button type="button" className="text-[var(--aq-blue-700)]" onClick={() => setMode("signin")}>Sign in</button> : null}
              {mode !== "signup" ? <button type="button" className="text-[var(--aq-blue-700)]" onClick={() => setMode("signup")}>Create account</button> : null}
              {mode !== "reset" ? <button type="button" className="text-[var(--aq-blue-700)]" onClick={() => setMode("reset")}>Forgot password?</button> : null}
            </div>
            <p className="mt-5 text-xs font-semibold text-[var(--aq-muted)]">
              By continuing, you agree to the <Link className="text-[var(--aq-blue-700)]" to="/terms">Terms</Link> and <Link className="text-[var(--aq-blue-700)]" to="/privacy">Privacy</Link>.
            </p>
          </CardContent>
        </Card>
      </section>
